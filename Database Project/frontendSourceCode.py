...
class ViewOrder(QtWidgets.QMainWindow):
    def __init__(self, parent, orderNumber):
        ...
        #############
        query = QSqlQuery('SELECT DISTINCT CustomerName FROM CUSTOMER'
        ...
        self.CustomerName = QtWidgets.QLineEdit(query.value(0))
        self.CustomerName.setReadOnly(True)
        #############
        query = QSqlQuery("SELECT DISTINCT CONCAT(FirstName, ' ', LastName) FROM CONTACT"
                        + " INNER JOIN JOB ON JOB.MainContact = CONTACT.ContactID"
                        + " INNER JOIN JOBORDER ON JOB.JobNumber = JOBORDER.JobNumber"
                        + " WHERE OrderNumber = {}".format(orderNumber))
        query.next()

        self.Contact = QtWidgets.QLineEdit(query.value(0))
        self.Contact.setReadOnly(True)
        #############
        query = QSqlQuery("SELECT JobName FROM JOB"
                        + " INNER JOIN JOBORDER ON JOB.JobNumber = JOBORDER.JobNumber"
                        + " WHERE OrderNumber = {}".format(orderNumber))
        query.next()

        self.JobName = QtWidgets.QLineEdit(query.value(0))
        self.JobName.setReadOnly(True)
        #############
        query = QSqlQuery("SELECT PONumber FROM JOB"
                        + " INNER JOIN JOBORDER ON JOB.JobNumber = JOBORDER.JobNumber"
                        + " WHERE OrderNumber = {}".format(orderNumber))
        query.next()

        self.PO = QtWidgets.QLineEdit(query.value(0))
        self.PO.setReadOnly(True)
        #############
        query = QSqlQuery("SELECT JobDate FROM JOB"
                        + " INNER JOIN JOBORDER ON JOB.JobNumber = JOBORDER.JobNumber"
                        + " WHERE OrderNumber = {}".format(orderNumber))
        query.next()

        # Formatting for date type variable
        self.JobDate = QtWidgets.QLineEdit(query.value(0).toString("MM/dd/yy"))
        self.JobDate.setReadOnly(True)
        #############
        query = QSqlQuery("SELECT EstimatedDueDate FROM JOB"
                        + " INNER JOIN JOBORDER ON JOB.JobNumber = JOBORDER.JobNumber"
                        + " WHERE OrderNumber = {}".format(orderNumber))
        query.next()

        # Formatting for date type variable
        self.EstimatedDueDate = QtWidgets.QLineEdit(query.value(0).toString("MM/dd/yy"))
        self.EstimatedDueDate.setReadOnly(True)
        #############
        query = QSqlQuery("SELECT AppliedFinish FROM JOB"
                        + " INNER JOIN JOBORDER ON JOB.JobNumber = JOBORDER.JobNumber"
                        + " WHERE OrderNumber = {}".format(orderNumber))
        query.next()

        self.Finish = QtWidgets.QLineEdit(query.value(0)) 
        # If there is no selected Finish value, 'None' is inserted
        if self.Finish.text() == '':
            self.Finish.insert('None')
        self.Finish.setReadOnly(True)
        #############
        query = QSqlQuery("SELECT CONCAT(Quantity, 'x ', HardwareName) FROM ORDERHARDWARE"
                        + " INNER JOIN JOBORDER ON ORDERHARDWARE.JobNumber = JOBORDER.JobNumber"
                        + " WHERE OrderNumber = {}".format(orderNumber))
        query.next()

        self.Hardware = QtWidgets.QLineEdit(query.value(0))
        ...
        query = QSqlQuery("SELECT DoorQuantity, DoorWidth, DoorHeight, MaterialName,"
        ...
        while query.next():
            ...
            for i in range(query.record().count()):
                self.table.setItem(rows, i, QTableWidgetItem(str(query.value(i))))
        ...
        query = QSqlQuery("SELECT DeliveryMethod FROM JOBORDER"
                        + " WHERE OrderNumber = {}".format(orderNumber))
        query.next()
        self.deliveryMethod = QtWidgets.QLineEdit(query.value(0))

        # If the Order has no specified DeliveryMethod, the DefaultDeliveryMethod is grabbed
        # from the Customer
        if self.deliveryMethod.text() == '':
            query = QSqlQuery("SELECT DefaultDeliveryMethod FROM CUSTOMER"
                        + " INNER JOIN JOB ON JOB.CustomerID = CUSTOMER.CustomerID"
                        + " INNER JOIN JOBORDER ON JOB.JobNumber = JOBORDER.JobNumber"
                        + " WHERE OrderNumber = {}".format(orderNumber))
            query.next()
        ...
        query = QSqlQuery("SELECT ShippingInstructions FROM JOBORDER"
                        + " WHERE OrderNumber = {}".format(orderNumber))
        query.next()
        self.shippingInstructions = QtWidgets.QTextEdit(query.value(0))
        self.shippingInstructions.setReadOnly(True)
        #############
        # Query to grab ProductionComments from the Order
        query = QSqlQuery("SELECT ProductionComments FROM JOBORDER"
                        + " WHERE OrderNumber = {}".format(orderNumber))
        query.next()
        self.productionComments = QtWidgets.QTextEdit(query.value(0))
...
class NewOrderWindow(QtWidgets.QMainWindow):
    def __init__(self, parent):
        ...
        query = QSqlQuery("SELECT TOP 1 OrderNumber FROM JOBORDER ORDER BY OrderNumber DESC")
        query.next()
        self.OrderNumber.setText(str(query.value(0) + 1))
        self.OrderNumber.setReadOnly(True)
        
        # Customer menu and CustomerID array is loaded with CustomerName and CustomerID from the database, respectively
        query = QSqlQuery("SELECT CustomerName, CustomerID FROM CUSTOMER")
        while query.next():
            self.CustomerName.addItem("{}".format(query.value(0)))
            self.CustomerID.append(query.value(1))
        ...
        # with a "None" option
        query = QSqlQuery("SELECT FinishName FROM FINISH")
        self.Finish.addItem("None")
        while query.next():
            self.Finish.addItem("{}".format(query.value(0)))

        # Hardware dropdown menu is loaded with HardwareName from the database
        # with a "None" option
        query = QSqlQuery("SELECT HardwareName FROM HARDWARE")
        self.Hardware.addItem("None")
        while query.next():
            self.Hardware.addItem("{}".format(query.value(0)))
    ...
    ##############################################################################
        ...
        query = QSqlQuery("SELECT DeliveryMethodName FROM PRODUCTDELIVERYMETHOD WHERE DeliveryMethodName <> 'WILL CALL'")
        self.deliveryMethod.addItem('WILL CALL')
        while query.next():
            self.deliveryMethod.addItem(query.value(0))
    ...
    def formatNewestRow(self):
        ...
        MaterialQuery = QSqlQuery("SELECT DISTINCT MaterialName FROM MATERIAL")
        ThicknessQuery = QSqlQuery("SELECT DISTINCT Thickness FROM MATERIAL")
        StyleCodeQuery = QSqlQuery("SELECT StyleCode FROM DOORSTYLE")
        InsideProfileQuery = QSqlQuery("SELECT ProfileName FROM DOORPROFILE WHERE ProfileType = 'I'")
        PanelProfileQuery = QSqlQuery("SELECT ProfileName FROM DOORPROFILE WHERE ProfileType = 'P'")
        OutsideProfileQuery = QSqlQuery("SELECT ProfileName FROM DOORPROFILE WHERE ProfileType = 'O'")
        BoreQuery = QSqlQuery("SELECT HingeBoringName FROM HINGEBORING")

        queries.append(MaterialQuery)
        queries.append(ThicknessQuery)
        queries.append(StyleCodeQuery)
        queries.append(InsideProfileQuery)
        queries.append(PanelProfileQuery)
        queries.append(OutsideProfileQuery)
        queries.append(BoreQuery)
        ...
        for j in range(3, 10):
            ...
            while queries[j-3].next():
                ch.addItem(queries[j-3].value(0))
    ...
    def updateThicknessStyleCode(self):
        ...
        thicknessQuery = QSqlQuery("SELECT DISTINCT Thickness FROM MATERIAL WHERE MaterialName = '{}'"
            .format(self.table.cellWidget(row, 3).currentText()))
        
        # Adds query values to new Thickness menu object
        while thicknessQuery.next():
            chNew.addItem(thicknessQuery.value(0))
        ...
        stylecodeQuery = QSqlQuery("SELECT DISTINCT StyleCode FROM STYLEPRICING WHERE MaterialName = '{}'"
            .format(self.table.cellWidget(row, 3).currentText()))
        
        # Adds query values to new StyleCode menu object
        while stylecodeQuery.next():
            chNew.addItem(stylecodeQuery.value(0))
    ...
    def updateStyleCode(self):
        ...
        # Query for getting StyleCode values based on Material and Thickness
        stylecodeQuery = QSqlQuery("SELECT StyleCode FROM STYLEPRICING WHERE MaterialName = '{}' AND Thickness = '{}'"
    ...
    def insertOrder(self):
        ...
        # QSqlQuery object for executing INSERT statements
        ...
        if not isEmpty:
            ...
            insertQuery.prepare("INSERT INTO JOB (CustomerID, MainContact, JobDate, EstimatedDueDate, JobName, PONumber, JobType, AppliedFinish, PriceModificationPerc)"
                            + " VALUES({}, {}, '{}', '{}', {}, {}, 'O', {}, {})"
                            .format(
            ...
            jobQuery = QSqlQuery("SELECT TOP 1 JobNumber FROM JOB ORDER BY JobNumber DESC")
            jobQuery.next()
            ...
            insertQuery.prepare("INSERT INTO JOBORDER (JobNumber, ProductionComments, InvoiceComments, ShippingInstructions, DeliveryMethod, OrderStatus)"
                            + " VALUES({}, {}, {}, {}, '{}', 'Processing')"
                            .format(jobNumber,
            ...
            for i in range(self.table.rowCount()):
                ...
                if (self.table.cellWidget(i, 0).text() == '' or self.table.cellWidget(i, 1).text() == '' or self.table.cellWidget(i, 2).text() == '' or
                    ...
                    insertQuery.prepare("INSERT INTO DOOR VALUES({}, {}, '{}', '{}', '{}', {}, {}, {}, {}, {}, {}, {}, {})"
                            .format(
                                    jobNumber,
                                    i + 1,
            ...
            if self.Hardware.currentText() != 'None':
                insertQuery.prepare("INSERT INTO ORDERHARDWARE VALUES({}, '{}', {})".format(jobNumber, self.Hardware.currentText(), hardwareNum))
                insertQuery.exec_()
    ...
    def updateContact(self):
        ...
        query = QSqlQuery("SELECT CONCAT(FirstName, ' ', LastName) AS Name, CONTACT.ContactID" 
        ...
        while query.next():
            self.Contact.addItem(query.value(0))
            self.ContactID.append(query.value(1))
...
class OrderListWindow(QtWidgets.QMainWindow):
    ...
    def updateCustomer(self):
        ...
        # database.
        query = QSqlQuery("SELECT DISTINCT CustomerName, CUSTOMER.CustomerID FROM CUSTOMER"
    ...
    def updateOrderNumber(self):
        ...
        while query.next():
            self.OrderNumber.addItem(str(query.value(0)))
    ...
    def updateTable(self):
        ...
        query = QSqlQuery("SELECT OrderNumber, CustomerName, CONCAT(FirstName, ' ', LastName) AS ContactName, JobName, PONumber, JobDate, EstimatedDueDate, OrderStatus" + 
                                " FROM JOB" + 
                                " INNER JOIN JOBORDER ON JOB.JobNumber = JOBORDER.JobNumber" +
        ...
        self.table.setColumnCount(query.record().count()) # Sets number of columns in the table to the number of columns in the query result
        ...
        while query.next():
            ...
            for i in range(query.record().count()):
                if type(query.value(i)) == type(PyQt5.QtCore.QDate()): # There is a Date type in the database, so that variable needs to be transformed using a .toString() function with a specified format
...
def createConnection():
    ...
    con.setDatabaseName("Driver={SQL Server Native Client 11.0};"
                        "Server=localhost;"
                        "Database=Ruckman_Riley_backend;"
